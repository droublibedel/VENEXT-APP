...
export type CompleteDetaillantOnboardingInput = {
  phone: string;
...
export type CompleteDetaillantOnboardingResult =
  | {
      ok: true;
...
export async function completeDetaillantOnboarding(
  input: CompleteDetaillantOnboardingInput,
): Promise<CompleteDetaillantOnboardingResult> {
  ...
  }>("/commerce-foundation/detaillant/register", {
    method: "POST",
    body: JSON.stringify({
      phone: input.phone,
