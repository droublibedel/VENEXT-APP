...
@Injectable()
export class RelationalStrategicObservatoryPolicyService {
  clampInt(n: number, min = 0, max = 100): number {
    if (!Number.isFinite(n)) return min;
    return Math.max(min, Math.min(max, Math.round(n)));
  }

  canMutateStrategicObservatoryState(corridorState: CommercialCorridorState): boolean {
    return (
  ...
  assertStrategicObservatoryMutationAllowed(corridorState: CommercialCorridorState): {
    allowed: boolean;
    diagnostics: {
      governanceOperation: "strategic_observatory_ingestion";
      mutationBlocked: boolean;
      strategicObservatoryMutationRejected: boolean;
      corridorTerminated: boolean;
      mutationSkippedReason: string | null;
    };
  } {
    const allowed = this.canMutateStrategicObservatoryState(corridorState);
    ...
    return {
      ...
      diagnostics: {
        ...
        strategicObservatoryMutationRejected: !allowed,
