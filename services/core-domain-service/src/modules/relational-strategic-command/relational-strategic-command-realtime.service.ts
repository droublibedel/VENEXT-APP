...
import {
  ...
  type RelationalStrategicCommandRealtimeEventType,
...
@Injectable()
export class RelationalStrategicCommandRealtimeService {
  private readonly log = new Logger(RelationalStrategicCommandRealtimeService.name);
  ...
  async publishToOrganizations(input: {
    ...
    intensity: number;
    commandDepth: number;
    eventType: RelationalStrategicCommandRealtimeEventType;
  }): Promise<void> {
  ...
  private async publishToOrg(input: {
    ...
    if (
      !(await this.flags.isEnabled("relational_strategic_command_realtime_enabled", {
        organizationId: input.organizationId,
      }))
    ) {
      return;
    }
    if (!isRelationalStrategicCommandRealtimeEventType(input.eventType)) return;
    const body = {
      relationshipId: input.relationshipId,
      commandNodeId: input.commandNodeId,
      nodeCode: input.nodeCode,
      intensity: input.intensity,
      commandDepth: input.commandDepth,
      computedAt: new Date().toISOString(),
      paymentExecutionDisabled: true as const,
      publicTrackingDisabled: true as const,
    };
    const parsed = RelationalStrategicCommandRealtimeSchema.safeParse(body);
    if (!parsed.success) {
