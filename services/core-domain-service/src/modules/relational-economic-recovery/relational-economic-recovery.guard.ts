...
@Injectable()
export class RelationalEconomicRecoveryGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) throw new ForbiddenException({ code: "relational_economic_recovery_missing_organization" });
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "relational_economic_recovery_org_mismatch" });
    }
