...
@Injectable()
export class RelationalEconomicRecoveryPolicyService {
  ...
  maxRecoveryDepth(): number {
    const raw = Number.parseInt(process.env.VENEXT_RECOVERY_MAX_DEPTH ?? "8", 10);
    if (!Number.isFinite(raw) || raw < 1) return 8;
    return Math.min(32, Math.max(1, raw));
  }

  canMutateRecoveryState(corridorState: CommercialCorridorState): boolean {
    return (
  ...
  assertEconomicRecoveryMutationAllowed(corridorState: CommercialCorridorState): {
    allowed: boolean;
    diagnostics: {
      governanceOperation: "economic_recovery_ingestion";
      mutationBlocked: boolean;
      recoveryMutationRejected: boolean;
      corridorTerminated: boolean;
      mutationSkippedReason: string | null;
    };
  } {
    const allowed = this.canMutateRecoveryState(corridorState);
    ...
    return {
      ...
      diagnostics: {
        ...
        recoveryMutationRejected: !allowed,
