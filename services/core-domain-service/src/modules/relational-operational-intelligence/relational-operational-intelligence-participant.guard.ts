...
/** Instruction 20.12 — operational intelligence: corridor party only (historical read on degraded/blocked). */
@Injectable()
export class RelationalOperationalIntelligenceParticipantGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) {
      throw new ForbiddenException({ code: "operational_intelligence_missing_organization" });
    }
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "operational_intelligence_org_mismatch" });
    }
