...
import { VENEXT_COMMERCE_THREAD_ACTOR_KEY } from "./commerce-thread-participant.guard";

/** Instruction 20.1B — negotiation-scoped routes (`:id` = negotiation UUID); actor from headers only. */
@Injectable()
export class NegotiationParticipantGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (
      resolved.organizationId !== neg.buyerOrganizationId &&
      resolved.organizationId !== neg.sellerOrganizationId
    ) {
