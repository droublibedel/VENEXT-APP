...
@Injectable()
export class RelationalPredictiveRiskParticipantGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) {
      throw new ForbiddenException({ code: "predictive_risk_missing_organization" });
    }
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "predictive_risk_org_mismatch" });
    }
