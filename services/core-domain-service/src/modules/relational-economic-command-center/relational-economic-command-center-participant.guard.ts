...
@Injectable()
export class RelationalEconomicCommandCenterParticipantGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) throw new ForbiddenException({ code: "economic_command_missing_organization" });
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "economic_command_org_mismatch" });
    }
