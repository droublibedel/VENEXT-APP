...
@Injectable()
export class RelationalStrategicIntelligenceGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) throw new ForbiddenException({ code: "relational_strategic_intelligence_missing_organization" });
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "relational_strategic_intelligence_org_mismatch" });
    }
