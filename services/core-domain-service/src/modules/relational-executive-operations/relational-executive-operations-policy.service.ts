...
export const VENEXT_EXECUTIVE_OPERATIONS_MAX_DEPTH = 6;

@Injectable()
export class RelationalExecutiveOperationsPolicyService {
  clampInt(n: number, min = 0, max = 100): number {
    if (!Number.isFinite(n)) return min;
    return Math.max(min, Math.min(max, Math.round(n)));
  }

  canMutateExecutiveOperationsState(corridorState: CommercialCorridorState): boolean {
    return (
  ...
  assertExecutiveOperationsMutationAllowed(corridorState: CommercialCorridorState): {
    allowed: boolean;
    diagnostics: {
      governanceOperation: "executive_operations_ingestion";
      mutationBlocked: boolean;
      executiveOperationsMutationRejected: boolean;
      corridorTerminated: boolean;
      mutationSkippedReason: string | null;
    };
  } {
    const allowed = this.canMutateExecutiveOperationsState(corridorState);
    ...
    return {
      ...
      diagnostics: {
        ...
        executiveOperationsMutationRejected: !allowed,
