...
@Injectable()
export class RelationalExecutiveStrategicSynthesisGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) throw new ForbiddenException({ code: "relational_executive_strategic_synthesis_missing_organization" });
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "relational_executive_strategic_synthesis_org_mismatch" });
    }
