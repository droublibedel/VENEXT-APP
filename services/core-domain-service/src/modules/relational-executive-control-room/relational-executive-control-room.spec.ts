...
const baseCtx: ExecutiveControlRoomCorridorContext = {
...
describe("RelationalExecutiveControlRoomPolicyService", () => {
  const policy = new RelationalExecutiveControlRoomPolicyService();

  it("blocks TERMINATED corridor mutations", () => {
    const gate = policy.assertExecutiveControlRoomMutationAllowed(CommercialCorridorState.TERMINATED);
    expect(gate.allowed).toBe(false);
    expect(gate.diagnostics.executiveControlRoomMutationRejected).toBe(true);
  });
});

describe("RelationalExecutiveControlRoomEngineService", () => {
  const policy = new RelationalExecutiveControlRoomPolicyService();
  const engine = new RelationalExecutiveControlRoomEngineService(
    policy,
    new RelationalExecutiveControlRoomPriorityService(policy),
    new RelationalExecutiveControlRoomRiskService(policy),
    new RelationalExecutiveControlRoomBalanceService(),
  );

  it("computes bounded executive operations scores", () => {
    const state = engine.computeExecutiveControlRoomState(baseCtx);
    expect(state.controlRoomScore).toBeLessThanOrEqual(100);
  ...
  it("detects coordination collapse under concentration stress", () => {
    ...
    const state = engine.computeExecutiveControlRoomState(stressed);
    expect(state.strategicCoordinationFailureDetected).toBe(true);
  });
});

describe("RelationalExecutiveControlRoomBoardService", () => {
  const policy = new RelationalExecutiveControlRoomPolicyService();
  const boardSvc = new RelationalExecutiveControlRoomBoardService(
    policy,
    new RelationalExecutiveControlRoomPriorityService(policy),
  );
  const engine = new RelationalExecutiveControlRoomEngineService(
  ...
  it("produces deterministic template grids from scores", () => {
    const state = engine.computeExecutiveControlRoomState(baseCtx);
    const boards = boardSvc.generateExecutiveControlRoomBoards(baseCtx, state);
    expect(boards.length).toBe(7);
    expect(boards[0]!.summary).toContain(String(state.controlRoomScore));
    expect(boards[0]!.institutionalPressure).toBeLessThanOrEqual(100);
  });
});
