...
@Injectable()
export class RelationalMacroEconomicPolicyService {
  ...
  canMutateMacroEconomicState(corridorState: CommercialCorridorState): boolean {
    return (
  ...
  assertMacroEconomicMutationAllowed(corridorState: CommercialCorridorState): {
    allowed: boolean;
    diagnostics: {
    ...
    const allowed = this.canMutateMacroEconomicState(corridorState);
  ...
  maxPropagationDepth(): number {
    const raw = Number.parseInt(process.env.VENEXT_MACRO_ECONOMIC_MAX_DEPTH ?? "8", 10);
