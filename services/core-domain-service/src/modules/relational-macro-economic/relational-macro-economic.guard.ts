...
@Injectable()
export class RelationalMacroEconomicGuard implements CanActivate {
  constructor(
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    if (!org) throw new ForbiddenException({ code: "relational_macro_economic_missing_organization" });
    const resolved = this.resolver.resolveFromRequest(req);
    if (resolved.organizationId !== org) {
      throw new ForbiddenException({ code: "relational_macro_economic_org_mismatch" });
    }
