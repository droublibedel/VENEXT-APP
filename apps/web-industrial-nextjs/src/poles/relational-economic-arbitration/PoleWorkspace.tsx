...
import { RelationalEconomicArbitrationPanel } from "./RelationalEconomicArbitrationPanel";

const SLUG = "relational-economic-arbitration" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const arbitrationEnabled = hydrated && flags.relational_economic_arbitration_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_economic_arbitration_realtime_enabled !== false;
    ...
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-fuchsia-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-fuchsia-200/90">
          {entry?.title ?? "Economic arbitration intelligence"}
        </p>
