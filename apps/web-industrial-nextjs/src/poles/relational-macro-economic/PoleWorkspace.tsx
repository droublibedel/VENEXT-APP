...
import { RelationalMacroEconomicPanel } from "./RelationalMacroEconomicPanel";

const SLUG = "relational-macro-economic" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const macroEnabled = hydrated && flags.relational_macro_economic_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_macro_economic_realtime_enabled !== false;
  ...
  return (
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-violet-800/50 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-violet-200/90">
          {entry?.title ?? "Macro-economic resilience"}
        </p>
        <p className="mt-1 text-[10px] text-violet-100/60">{entry?.subtitle}</p>
      </header>
