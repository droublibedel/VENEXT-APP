...
import { RelationalEconomicSignalGraphPanel } from "./RelationalEconomicSignalGraphPanel";

const SLUG = "relational-economic-signal-graph" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const graphEnabled = hydrated && flags.relational_economic_signal_graph_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_economic_signal_graph_realtime_enabled !== false;
  ...
  const lastRealtimeEvent = useMemo(() => {
    const hit = realtimeGateway.stream.find(
      (s) => s.detail.includes("economic") || s.label.includes("economic"),
    );
  ...
  return (
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-amber-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-amber-200/90">
          {entry?.title ?? "Signaux économiques"}
        </p>
