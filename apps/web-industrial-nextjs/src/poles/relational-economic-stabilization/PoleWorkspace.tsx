...
import { RelationalEconomicStabilizationPanel } from "./RelationalEconomicStabilizationPanel";

const SLUG = "relational-economic-stabilization" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const stabilizationEnabled = hydrated && flags.relational_economic_stabilization_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_economic_stabilization_realtime_enabled !== false;
    ...
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-teal-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-teal-200/90">
          {entry?.title ?? "Economic stabilization intelligence"}
        </p>
