...
import { RelationalOperationalIntelligencePanel } from "./RelationalOperationalIntelligencePanel";

const SLUG = "relational-operational-intelligence" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const gatewayEnabled =
    ...
    flags.relational_operational_intelligence_enabled !== false &&
    Boolean(entry);
    ...
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-cyan-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-cyan-200/90">
          {entry?.title ?? "Intelligence opérationnelle"}
        </p>
      ...
      <RelationalOperationalIntelligencePanel
        ...
        intelligenceEnabled={hydrated && flags.relational_operational_intelligence_enabled !== false}
      />
