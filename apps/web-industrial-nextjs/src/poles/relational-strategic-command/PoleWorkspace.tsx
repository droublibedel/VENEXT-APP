...
import { RelationalStrategicCommandPanel } from "./RelationalStrategicCommandPanel";

const SLUG = "relational-strategic-command" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const commandEnabled = hydrated && flags.relational_strategic_command_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_strategic_command_realtime_enabled !== false;
    ...
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-violet-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-violet-200/90">
          {entry?.title ?? "Strategic command center"}
        </p>
