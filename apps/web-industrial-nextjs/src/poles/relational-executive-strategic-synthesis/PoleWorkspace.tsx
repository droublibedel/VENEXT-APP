...
import { RelationalExecutiveStrategicSynthesisPanel } from "./RelationalExecutiveStrategicSynthesisPanel";

const SLUG = "relational-executive-strategic-synthesis" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const synthesisEnabled = hydrated && flags.relational_executive_strategic_synthesis_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_executive_strategic_synthesis_realtime_enabled !== false;
    ...
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-violet-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-violet-200/90">
          {entry?.title ?? "Synthèse exécutive stratégique"}
        </p>
