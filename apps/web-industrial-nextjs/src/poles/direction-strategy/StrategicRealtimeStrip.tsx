...
export function StrategicRealtimeStrip({
  connected,
  demoMode,
  liveChannel,
  latest,
  ...
  return (
    <div className="mx-2 mb-3 rounded border border-slate-800/90 bg-slate-950/90 px-3 py-2 text-[11px] text-slate-300">
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-800/80 pb-2">
        <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-amber-200/90">
          Strategic realtime
        </span>
        <span
          className={`rounded px-1.5 py-0.5 font-mono text-[10px] ${demoMode ? "bg-amber-900/40 text-amber-100" : "bg-emerald-900/40 text-emerald-100"}`}
        >
          {modeLabel}
        </span>
      ...
      {latest ? (
        ...
        <p className="mt-2 text-slate-600">Awaiting batched operational signals (same gateway cap as pole canvas).</p>
      )}
    </div>
  );
}
