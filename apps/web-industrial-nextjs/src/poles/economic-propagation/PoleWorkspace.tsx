...
import { EconomicPropagationWorkspace } from "./EconomicPropagationWorkspace";
import { resolveEconomicPropagationOrganizationId } from "./resolveEconomicPropagationOrganizationId";

const SLUG = "economic-propagation" as const satisfies PoleSlug;

export default function PoleWorkspace() {
  const org = resolveEconomicPropagationOrganizationId();
  ...
  return (
    <div className="flex min-h-0 flex-col">
      <EconomicPropagationWorkspace realtimeGateway={realtimeGateway} organizationResolution={org} />
      <OperationalPoleCanvas
        poleSlug={SLUG}
        realtimeGateway={realtimeGateway}
        economicPropagationOrganizationId={org.organizationId}
      />
    </div>
  );
}
