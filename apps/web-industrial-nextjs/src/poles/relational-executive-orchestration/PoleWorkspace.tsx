...
import { RelationalExecutiveOrchestrationPanel } from "./RelationalExecutiveOrchestrationPanel";

const SLUG = "relational-executive-orchestration" as const satisfies PoleSlug;
...
export default function PoleWorkspace() {
  ...
  const orchestrationEnabled = hydrated && flags.relational_executive_orchestration_enabled !== false;
  const realtimeEnabled = hydrated && flags.relational_executive_orchestration_realtime_enabled !== false;
  ...
  return (
    <div className="flex min-h-0 flex-col gap-3 px-3 py-3">
      <header className="rounded border border-fuchsia-900/40 bg-slate-950/90 px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-fuchsia-200/90">
          {entry?.title ?? "Executive economic orchestration"}
        </p>
