...
export function isEconomicCommandDemoMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
...
export type EconomicCommandBffHeaderInput = {
  searchParams: URLSearchParams;
  getHeader: (name: string) => string | null;
};

export function buildEconomicCommandUpstreamHeadersFromInput(
  input: EconomicCommandBffHeaderInput,
  ...
  if (isEconomicCommandDemoMode(env)) {
  ...
  return {
    status: 403,
    body: {
      code: "economic_command_bff_actor_required",
      message:
        "Economic command BFF requires Authorization, VENEXT_SERVER_ACTOR_USER_ID + VENEXT_SERVER_ACTOR_ORG_ID, or demo mode. Browser-supplied actor headers are not accepted as production authentication.",
    },
  };
}

export function buildEconomicCommandUpstreamHeaders(req: NextRequest): Headers | Response {
  const r = buildEconomicCommandUpstreamHeadersFromInput({
...
const CORE_HINT =
  "Set CORE_DOMAIN_URL or NEXT_PUBLIC_CORE_DOMAIN_URL for /api/economic-command proxy targets.";

export async function proxyEconomicCommandRequest(req: NextRequest, pathSegments: string[]): Promise<Response> {
  const base = resolveCoreUpstream();
  if (!base) {
    return Response.json({ error: "Upstream not configured", hint: CORE_HINT }, { status: 503 });
  }
  const headersOrErr = buildEconomicCommandUpstreamHeaders(req);
