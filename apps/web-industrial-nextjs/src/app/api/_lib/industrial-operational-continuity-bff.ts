...
export function isIndustrialOperationalContinuityDemoMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
...
export function buildIndustrialOperationalContinuityUpstreamHeaders(req: NextRequest): Headers | Response {
  const h = new Headers();
  const corr = req.headers.get("x-correlation-id");
  if (corr) h.set("x-correlation-id", corr);
  const orgQ = req.nextUrl.searchParams.get("organizationId");

  if (isIndustrialOperationalContinuityDemoMode()) {
  ...
  return Response.json(
    {
      code: "industrial_operational_continuity_bff_actor_required",
      message:
        "Industrial operational continuity BFF requires Authorization, VENEXT_SERVER_ACTOR_USER_ID + VENEXT_SERVER_ACTOR_ORG_ID, or demo mode.",
    },
    { status: 403 },
  );
}
...
const CORE_HINT =
  "Set CORE_DOMAIN_URL or NEXT_PUBLIC_CORE_DOMAIN_URL for /api/industrial-operational-continuity proxy targets.";

export async function proxyIndustrialOperationalContinuityRequest(
  req: NextRequest,
  pathSegments: string[],
): Promise<Response> {
  ...
  const headersOrErr = buildIndustrialOperationalContinuityUpstreamHeaders(req);
