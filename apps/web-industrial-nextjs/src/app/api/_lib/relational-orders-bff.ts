...
export function isRelationalOrdersDemoMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
...
export function buildRelationalOrdersUpstreamHeaders(req: NextRequest): Headers | Response {
  const h = new Headers();
  const corr = req.headers.get("x-correlation-id");
  if (corr) h.set("x-correlation-id", corr);
  const orgQ = req.nextUrl.searchParams.get("organizationId");

  if (isRelationalOrdersDemoMode()) {
  ...
  return Response.json(
    {
      code: "relational_orders_bff_actor_required",
      message:
        "Relational orders BFF requires Authorization, VENEXT_SERVER_ACTOR_USER_ID + VENEXT_SERVER_ACTOR_ORG_ID, or demo mode.",
    },
    { status: 403 },
  );
}
...
const CORE_HINT =
  "Set CORE_DOMAIN_URL or NEXT_PUBLIC_CORE_DOMAIN_URL for /api/relational-orders proxy targets.";

export async function proxyRelationalOrdersRequest(req: NextRequest, pathSegments: string[]): Promise<Response> {
