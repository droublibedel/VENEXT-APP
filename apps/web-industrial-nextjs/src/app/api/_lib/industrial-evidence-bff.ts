...
export function isIndustrialEvidenceDemoMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
...
export function buildIndustrialEvidenceUpstreamHeaders(req: NextRequest): Headers | Response {
  const h = new Headers();
  const corr = req.headers.get("x-correlation-id");
  if (corr) h.set("x-correlation-id", corr);
  const orgQ = req.nextUrl.searchParams.get("organizationId");

  if (isIndustrialEvidenceDemoMode()) {
  ...
  return Response.json(
    {
      code: "industrial_evidence_bff_actor_required",
      message:
        "Industrial evidence BFF requires Authorization, VENEXT_SERVER_ACTOR_USER_ID + VENEXT_SERVER_ACTOR_ORG_ID, or demo mode.",
    },
    { status: 403 },
  );
}
...
const CORE_HINT =
  "Set CORE_DOMAIN_URL or NEXT_PUBLIC_CORE_DOMAIN_URL for /api/industrial-evidence proxy targets.";

export async function proxyIndustrialEvidenceRequest(req: NextRequest, pathSegments: string[]): Promise<Response> {
  const base = resolveCoreUpstream();
  if (!base) {
    return Response.json({ error: "Upstream not configured", hint: CORE_HINT }, { status: 503 });
  }
  const headersOrErr = buildIndustrialEvidenceUpstreamHeaders(req);
