...
export const GrossisteProductCard = memo(function GrossisteProductCard({
  ...
  return (
    <article className="grossiste-b-card" data-testid={`grossiste-product-${product.id}`}>
      ...
      <p style={{ margin: "10px 0 0", fontSize: 16, fontWeight: 700, color: "#f0f4f2" }}>{product.priceLabel}</p>
      {product.promotion ? (
        <p style={{ margin: "6px 0 0", fontSize: 12, color: "#00a884" }}>{product.promotion}</p>
      ) : null}
      <p style={{ margin: "6px 0 0", fontSize: 11, color: "#6b8078" }}>
